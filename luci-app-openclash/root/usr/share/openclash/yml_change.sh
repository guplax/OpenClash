...
begin
   ...
   default_proxy_servers = ['112.215.198.248', '112.215.203.248', '112.215.198.254', '112.215.203.254', '8.8.8.8', '8.8.4.4']
   ...
   if Value.dig('dns', 'proxy-server-nameserver').to_a.empty?
      ...
      if respect_rules || Value.dig('dns', 'respect-rules').to_s == 'true' || all_ns_proxied
         ...
         if all_ns_proxied
            YAML.LOG('Tip: Nameserver Option Maybe All Setted The Proxy Option, Auto Set Proxy-server-nameserver Option to【112.215.198.248, 112.215.203.248, 112.215.198.254, 112.215.203.254, 8.8.8.8, 8.8.4.4】For Avoiding Proxies Server Resolve Loop...')
         else
            YAML.LOG('Tip: Respect-rules Option Need Proxy-server-nameserver Option Must Be Setted, Auto Set to【112.215.198.248, 112.215.203.248, 112.215.198.254, 112.215.203.254, 8.8.8.8, 8.8.4.4】')
         end
      end
   else
      ...
      if all_psn_proxied
         ...
         YAML.LOG('Tip: Proxy-server-nameserver Option Maybe All Setted The Proxy Option, Auto Set Proxy-server-nameserver Option to【112.215.198.248, 112.215.203.248, 112.215.198.254, 112.215.203.254, 8.8.8.8, 8.8.4.4】For Avoiding Proxies Server Resolve Loop...')
      end
   end
